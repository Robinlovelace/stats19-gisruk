...
```{r, fig.cap="Crash severity by speed limit (top) and crash severity by age band of casualty (bottom)", fig.height=6}
...
```{r, fig.cap="An approximate geographical layout of England and Wales police force areas following the layout algorithm published in @meulemans_small_2017", echo=FALSE, fig.height=2.5, fig.width=7}
library(png)
library(grid)
img <- readPNG("./semispatial_layout.png")
 grid.raster(img)
```

```{r, fig.cap="Risk ratios of KSI rates by force area averaged over 2013-2017 (left and middle), with year-on-year trend in risk ratios and raw KSIs (right)", echo=FALSE, fig.height=3.5, fig.width=7}
library(png)
library(grid)
img <- readPNG("./risk_ratio_maps.png")
 grid.raster(img)
```
